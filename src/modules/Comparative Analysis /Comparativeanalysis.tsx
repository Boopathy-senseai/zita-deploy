...
import { comparativeanalysismiddleware, comparativesearchingdatamiddleware } from './store/middleware/comparativemiddleware';
...
const ComparativeanalysisModal = ({
  ...
  const dispatchcomparativeApi = () => {
    ...
    dispatch(comparativeanalysismiddleware({ candidate_ids: candidateids, job_id: jdId, categories: selectcriteria })).then((response) => {
        ...
        {isLoader ? <Loader /> :
          <Modal open={Comparative}>
            ...
            <Flex style={{ backgroundColor: 'white' }}>
              ...
              <Flex className={styles.fixingcontent} height={window.innerHeight - 70} >
                <Card className={styles.card}>
                  ...
                  {selectedcriteria ? <Flex className={styles.container}>
                      ...
                      <Flex style={{ justifyContent: 'center' }}>
                        <Flex middle>
                          <Avatar
                            ...
                            avatar={
                              selectedcriteria.payload.analysis[0].image && selectedcriteria.payload.analysis[0].image !== 'default.jpg'
                                ? `${process.env.REACT_APP_HOME_URL}media/${selectedcriteria.payload.analysis[0].image}`
                                : undefined

                            }
                            initials={`${selectedcriteria.payload.analysis[0]?.first_name?.charAt(0)}
                          ${!isEmpty(selectedcriteria.payload.analysis[0].last_name)
                                ? selectedcriteria.payload.analysis[0].last_name?.charAt(0)
                                : ''
                              }`}
                          />
                        </Flex>
                        <Text style={{ padding: '2px 0px 0px 0px' }}>{`${selectedcriteria.payload.analysis[0]?.first_name}${!isEmpty(selectedcriteria.payload.analysis[0].last_name)
                ...
                <Flex row flex={12}>
                  <Flex>
                    ...
                    {selectedcriteria ? <>
                      {selectedcriteria.payload.analysis[0].categories.map((item, index) => (
                        <Flex
                          marginTop={6}
                          style={{
                            borderBottom: '1px solid rgb(195, 195, 195)',
                  ...
                  >
                    {selectedcriteria && selectedcriteria.payload.analysis.map((e, indexnum) => {
                      return (
                        <Flex
                          key=""
                          row
                        ...
                        >
                          <Card className={styles.cardstructureforprofile}>
                            <Flex row between>
                              ...
                              >
                                <Flex center middle>
                                  <text style={{ color: 'white' }}>{indexnum + 1}</text>
                                </Flex>
                              ...
                              >
                                ...
                                <Flex
                                  className={cx({
                                    countStyle1: e.Total_matching_percentage < 40,
                                    countStyle2:
                                      e.Total_matching_percentage >= 40 &&
                                      e.Total_matching_percentage < 69,
                                    countStyle3: e.Total_matching_percentage > 69,
                                  })}
                                >
                            ...
                            >
                              {' '}
                              <StarsRating value={e.overall_scorecard} disabled count={5} />
                            </Flex>
                            <Flex marginTop={20}>
                              {e.categories.map((item, index) => (
                                <Flex key={index} center middle height={34} style={{
